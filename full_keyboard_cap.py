...
Logged action keys/buttons (held-fraction 0.0-1.0 per frame window):
  every keyboard key (letters, digits, punctuation, function keys,
  arrows, numpad, modifiers, etc.) plus lmb/rmb -- see
  TRACKED_ACTION_NAMES / key_to_name() below. Insert and Escape are the
  only two keys excluded from logging, since they're reserved for
  session control (see Controls above).
...
# Insert/Escape stay reserved for session control (see SessionController)
# and are never logged as action keys, so pressing them to start/pause/
...
# function keys, home/end/page up/down, caps lock, the Windows/cmd
# key, media keys if present, etc. -- gets its own column, named after
...
SPECIAL_KEY_MAP = {
    key: _MODIFIER_MERGE.get(key.name, key.name)
    for key in keyboard.Key
    if key not in CONTROL_KEYS
}

# Character keys (letters/digits/punctuation) arrive as KeyCode, not
# Key, and key.char on those is modifier-sensitive -- e.g. holding
# Ctrl turns "w" into the control character "\x17" instead of "w", and
# some layouts remap punctuation entirely. key.vk (the raw Windows
# virtual-key code) stays stable regardless of Shift/Ctrl/AltGr state,
# so we key off that instead. VK codes below assume a US keyboard
# layout -- relabel this map if you're on a different one.
VK_NAME_MAP = {}
for _vk in range(0x30, 0x3A):   # '0'-'9'
    VK_NAME_MAP[_vk] = chr(_vk)
for _vk in range(0x41, 0x5B):   # 'A'-'Z'
    VK_NAME_MAP[_vk] = chr(_vk).lower()
VK_NAME_MAP.update({
    0xBA: "semicolon", 0xBB: "equals", 0xBC: "comma", 0xBD: "minus",
    0xBE: "period", 0xBF: "slash", 0xC0: "backtick", 0xDB: "lbracket",
    0xDC: "backslash", 0xDD: "rbracket", 0xDE: "quote",
})
for _i in range(10):             # numpad 0-9
    VK_NAME_MAP[0x60 + _i] = f"numpad{_i}"
VK_NAME_MAP.update({
...
def key_to_name(key):
    ...
    Insert/Escape are consumed by SessionController for session control
    (start/pause/stop) and are deliberately excluded here, so they
    never show up as a logged column."""
...
TRACKED_ACTION_NAMES = sorted(
    set(VK_NAME_MAP.values()) | set(SPECIAL_KEY_MAP.values()) | set(MOUSE_BUTTON_MAP.values())
)
